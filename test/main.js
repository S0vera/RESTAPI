...
describe("ToDo Application", async function () {
  ...
  it("/ should render correctly render header with 'There is no tasks' text when task list is empty on init", function () {
    ...
    expect(noTasksTitle.first().getText()).to.eventually.be.equal(
      "There is no tasks."
    );
  });
  ...
  it("/ form should validate input. min length is 3.", async function () {
    ...
    expect(
      await alertsOnInit.count(),
      "It should not render alerts on blank page render"
    ).to.equal(0);

    const formInput = $(".task-form .input");
    const formButton = $(".task-form .task-form__button");

    expect(formInput.isPresent(), "Form input should exists.").to.be.eventually
      .true;
    expect(formButton.isPresent(), "Form button should exists.").to.be
      .eventually.true;
    ...
    expect(
      await alertsAfterReload.count(),
      "It should not render alerts on page reload"
    ).to.equal(0);
  });
  ...
  it("/ form should correctly add task and show success message", async function () {
    ...
    expect(await formButton.isPresent(), "Form button should exists.").to.be
      .true;
    ...
    await browser.wait(
      ...
      10000,
      "An success alert was not rendered in 10000 seconds. Test timed out!"
    );
    ...
    expect(
      await tasks.count(),
      "It should render 1 task after 1 form submission"
    ).to.be.equal(1);
    expect(
      await tasks.getText(),
    ...
    const checkboxClass = await tasks
      .first()
      .$(".checkbox")
      .getAttribute("class");
    ...
    expect(
      await alerts.count(),
      "It should render 1 success alert"
    ).to.be.equal(1);
    expect(
  ...
  it("/ form should correctly add multiple tasks and validate that task does not exists.", async function () {
    ...
    await browser.wait(
      ...
      10000,
      "Two tasks was not rendered in 10000 seconds. Test timed out!"
    );
    ...
    await browser.wait(
      ...
      10000,
      "An error alert was not rendered in 10000 seconds. Test timed out!"
    );
  ...
  it("/ should remove ticket when clicked on checkbox and move it to DONE page", async function () {
    ...
    await browser.wait(
      ...
      10000,
      "Message was not removed in 10000 seconds. Timeout exceeded!"
    );
  ...
  it("/done should render correct done tasks", async function () {
    ...
    expect(await doneTasks.count(), "It should render done tasks").to.be.equal(
      1
    );
